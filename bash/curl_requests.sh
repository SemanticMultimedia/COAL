...
do
    if [ $LOCAL = "true" ]
    then curl -v  -H "accept:text/turtle" "http://localhost:8080/coal/resource?url=http://static.nico.is/testpodcast.mp3";
	else curl -v  -H "accept:text/turtle" "http://172.16.65.75:8080/coal/resource?url=http://static.nico.is/testpodcast.mp3"
	fi
	sleep 5
done
